...
- ### 1). 파이썬 Flask 서버와 자바의 jsp 사이의 데이터 전송에 대해  언어가 서로 달라 어떻게 <br>
  ...
  **해결 방안): 간단하게 플라스크서버 주소로 get요청을보내 데이터를 실어 보내어 간단하게 해결 할수 있었습니다.** 
  <br>
...
- ### 2). 장바구니페이지에서 선택한 제품만의 수량 정보만 최종 주문/결제 페이지로 이동되지 않고 모든 수량 정보가 도착함.
   ...
   **해결 방안):** 이를 구분해주기위해 서버에서 다시한번 장바구니 목록 데이터를 불러와, **이중for문 돌려 선택한 체크박스**의<br>
   **장바구니 식별자와 일치하는 순간의 인덱스번호로 수량이 담긴 배열에서도 동일한 인덱스번호 값**에 해당한 <br>
